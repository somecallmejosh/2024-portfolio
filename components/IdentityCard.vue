<template>
  <div class="bg-white  sticky top-20 pt-12 rounded-xl">
    <div class="w-56 h-48 mx-auto bg-gray-50 -translate-y-24 rounded-2xl hover:blur transition-all duration-500 relative group">
      <img class="w-56 h-48 flex-none rounded-2xl object-cover relative z-0" src="https://www.logatot.com/assets/team/josh-95a85dfb5617d380bc60162eafa2d6fe6413112b22c35f161d6589f11580eb34.jpg" alt="Joshua Briley, UX/UI Developer">
    </div>
    <div class="-mt-14">
      <div class="text-center px-4 mb-10">
        <h2 class="text-xl font-bold mb-1">Joshua Briley</h2>
        <p class="text-gray-500 font-medium">UI/UX Developer</p>
      </div>
      <ul class="text-left px-12 pt-10 pb-12 bg-slate-50 space-y-4 text-gray-600 font-medium rounded-b-2xl">
        <li class="flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24"><path d="M480-480q33 0 56.5-23.5T560-560q0-33-23.5-56.5T480-640q-33 0-56.5 23.5T400-560q0 33 23.5 56.5T480-480Zm0 294q122-112 181-203.5T720-552q0-109-69.5-178.5T480-800q-101 0-170.5 69.5T240-552q0 71 59 162.5T480-186Zm0 106Q319-217 239.5-334.5T160-552q0-150 96.5-239T480-880q127 0 223.5 89T800-552q0 100-79.5 217.5T480-80Zm0-480Z"/></svg>
          <div class="sr-only">I live in</div>
          Avon, CT
        </li>
        <li class="flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24"><path d="M160-160q-33 0-56.5-23.5T80-240v-480q0-33 23.5-56.5T160-800h640q33 0 56.5 23.5T880-720v480q0 33-23.5 56.5T800-160H160Zm320-280L160-640v400h640v-400L480-440Zm0-80 320-200H160l320 200ZM160-640v-80 480-400Z"/></svg>
          <div class="sr-only">Email me at</div>
          josh@thebrileys.com
        </li>
        <li class="flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24"><path d="M280-40q-33 0-56.5-23.5T200-120v-720q0-33 23.5-56.5T280-920h400q33 0 56.5 23.5T760-840v720q0 33-23.5 56.5T680-40H280Zm0-200v120h400v-120H280Zm200 100q17 0 28.5-11.5T520-180q0-17-11.5-28.5T480-220q-17 0-28.5 11.5T440-180q0 17 11.5 28.5T480-140ZM280-320h400v-400H280v400Zm0-480h400v-40H280v40Zm0 560v120-120Zm0-560v-40 40Z"/></svg>
          <div class="sr-only">Call me at</div>
          (860) 232-8250
        </li>
      </ul>
    </div>
  </div>
</template>
